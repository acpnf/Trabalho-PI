...
// MUDANÇA: Adicionado o estado StartScreen para a tela inicial
typedef enum
{
    StartScreen,
    Menu,
...
int foo(double currentTime, Inimigo *inimigo, int index, int curr_index)
{
    ...
    if (index <= curr_index && (currentTime - lastMoveTime) >= moveInterval) 
    {
        ...
        if (inimigo->posX == 1 && inimigo->posY == 17 && inimigo->morto == false) {
...
int main(void) {
    ...
    while (!WindowShouldClose() && !sair) {
        ...
        switch (estadoJogo)
        {
            // MUDANÇA: Novo case para a tela inicial
            case StartScreen:
            {
                BeginDrawing();
                ClearBackground(RAYWHITE);

                // Desenha a imagem da tela inicial
                DrawTexturePro(startScreenImage[indiceStartImagem], (Rectangle){0.0, 0.0, startScreenImage[indiceStartImagem].width, startScreenImage[indiceStartImagem].height}, (Rectangle){0, 0, largura, altura},
                    (Vector2){0.0f, 0.0f}, 0.0f, WHITE);

                // Se a tecla de espaço for pressionada, muda para o estado Jogando
...
fim_do_jogo:
    ...
    UnloadTexture(startScreenImage[0]);
    UnloadTexture(startScreenImage[1]);
